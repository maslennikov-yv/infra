...
`make backup` (или `make redis-backup` из корня) создаёт **`redis/backups/redis-backup-YYYYMMDD-HHMMSS.tar.gz`** с:
...
```bash
make restore-acl BACKUP_FILE=backups/redis-backup-20260508-143022.tar.gz ENV=local
...
2. **Скопировать `dump.rdb` в PVC через временный pod.** PVC называется `redis-data-redis-master-0` (по умолчанию).
   ...
   tar -xzf redis/backups/redis-backup-YYYYMMDD-HHMMSS.tar.gz -C /tmp dump.rdb
...
4. **Применить ACL из того же бэкапа**:
   ```bash
   make redis-restore-acl BACKUP_FILE=redis/backups/redis-backup-YYYYMMDD-HHMMSS.tar.gz ENV=local
   ```
...
Ротация — стандартная (`logrotate` для логов; для архивов — `find redis/backups -name 'redis-backup-*.tar.gz' -mtime +14 -delete`).
