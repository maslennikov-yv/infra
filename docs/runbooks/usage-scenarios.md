...
1. Комплексная сводка: `make doctor ENV=…` (тулинг + доступность кластера + helm vs helmfile + rollouts + per-app verify).
2. В каталоге сервиса: `make status ENV=…` ([README — команды каталога](../../README.md#команды-для-отдельного-сервиса)).
3. По кластеру: цели вида `monitoring-top-nodes`, `monitoring-pod-events` (см. [Рекомендации и проверки](../../README.md#рекомендации-по-железу) и соседние абзацы в README про Pending и описание пода).
4. Постоянный слой наблюдаемости: [Мониторинг (Netdata)](../../README.md#мониторинг-netdata), сервис `netdata` в списке `ENABLED_SERVICES`.

### Через infra-lab

1. Сводка: **Управление → Среда → Кластер и доступ → «Полная диагностика стека (doctor)»**.

2. Статус, логи, shell по data-сервису: **Управление → Сервис → Диагностика сервисов** → выбрать компонент → **«Состояние подов и релиза»** (`postgres-status`, `redis-status`, …) / **«Логи»** / **«Shell в контейнере»**. Для PostgreSQL дополнительно: **«Бэкапы, восстановление и PVC»**; для Redis / Kafka / MinIO / ClickHouse / RabbitMQ — **«Бэкапы и восстановление»**.

3. Netdata как отдельный пункт списка: снова **Диагностика сервисов → Netdata (мониторинг)** → те же действия; **«Проброс порта 19999…»**; **«Ещё: топ узлов, события, Helm»** — оттуда **«Топ узлов по ресурсам»** (`monitoring-top-nodes`), события pod, describe pod, подменю Helm Netdata (`monitoring-up` / `monitoring-diff` / `monitoring-down`).

4. Helm только для Netdata: **Управление → Сервис → Helm… → Один компонент** → **Netdata (мониторинг)** → нужное действие.

5. Включить Netdata **в общем** `up`: мультивыбор в [глобальном Helm](#infra-lab-общие-замечания) с отметкой Netdata. Настройка ingress и `values` — в git, не в TUI.
...
4. Sanity: **Управление → Среда → Кластер и доступ → «Полная диагностика стека (doctor)»**.
...
- Один сервис: **Управление → Сервис → Диагностика сервисов → `<сервис>` → Бэкапы и восстановление** (для PostgreSQL пункт расширенный — «Бэкапы, восстановление и PVC»). При **Восстановить из архива** запрашивается `BACKUP_FILE` и явное подтверждение.
- Все сразу: **Управление → Среда → «Бэкап всех stateful-сервисов разом (backup-all)»** — учитывает `ENABLED_SERVICES` / `EXCLUDE_SERVICES`.
- Архив окружения: **Управление → Среда → «Сохранить секреты и манифесты окружения в архив»** (`env-backup`); восстановить — соседний пункт **«Восстановить окружение из архива (опасно)»** (`env-restore`).
- Расписание (cron / systemd timer / CI) задаётся в системе оператора, не в TUI.
