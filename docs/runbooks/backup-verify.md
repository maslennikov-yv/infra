...
- [`secrets-management.md`](secrets-management.md) — sops+age для `apps/conf/<APP>/secrets.enc.yaml`.
...
Зачем: env-restore из prod-снимка попытается записать `apps/conf/<APP>/secrets.yaml` с **prod creds**, что перетрёт ваши локальные creds. Swap делает откат тривиальным.
...
- **`OVERWRITE_APPS_CONF=1`**: перетирает существующие `apps/conf/<APP>/` (backup-verify нуждается в prod-creds для запуска под ними). Без флага — пропускает существующие.
...
Подменяет в restored Secret `<APP>-minio` и `apps/conf/<APP>/secrets.yaml`:
...
Приводит SCRAM/ACL/IAM creds к состоянию, заявленному в `apps/conf/<APP>/secrets.yaml` (восстановленному из prod env-backup). Это важно потому что:
