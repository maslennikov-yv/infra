...
[docs/runbooks/usage-scenarios.md](runbooks/usage-scenarios.md) — 7 сценариев в порядке частоты:
бутстрап → новое приложение → ротация секретов → здоровье → бэкапы → конфигурирование сервисов → отключение приложения.
